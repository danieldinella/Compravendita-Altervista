...
<!NAVBAR>
  ...
  if($tipo=='Acquisto'){
      ...
      $Query= "SELECT ID_Titolo, Prezzo, ID_Cliente, Tipo FROM RICHIESTE WHERE ID_Richiesta=$id_r ;";
    $Res= mysqli_query($Connessione, $Query); //Eseguire la query
    $Dati=mysqli_fetch_array($Res);
    $ammontare=$Dati['Prezzo'];
    $id_t=$Dati['ID_Titolo'];
    if($Dati['ID_Cliente']==$id or $Dati['Tipo']=='Acquisto' ){
        print("<h2 style=\"background-color:#fe5d00;\">Transazione non disponibile<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
        exit;
    }
    
  }else{
      $ammontare=$_POST['Ammontare'];
      $id_t=$_POST['ID_T'];
      $Query= "SELECT ID_Cliente FROM TITOLI WHERE ID_Titolo=$id_t ;";
    $Res= mysqli_query($Connessione, $Query); //Eseguire la query
    $Dati=mysqli_fetch_array($Res);
    if($Dati['ID_Cliente']!=$id){
        print("<h2 style=\"background-color:#fe5d00;\">Titolo non disponibile<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
        exit;
    }
  }
  //Impostare la query

  $Query= "INSERT INTO TRANSAZIONI VALUES (NULL,NULL,$ammontare,'$tipo','$fido',$id,NULL,$id_t);";

  $Res= mysqli_query($Connessione, $Query); //Eseguire la query

  if(!$Res)
  print("<h2 style=\"background-color:#fe5d00;\">Transazione fallita<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
  else
  print("<h2 style=\"background-color:#fe5d00;\">Transazione proposta con successo<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
  mysqli_close($Connessione); //Chiudere la connessione
  ?>
