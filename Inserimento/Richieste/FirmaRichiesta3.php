...
<!NAVBAR>
  ...
  $data=$_POST['Data'];
   $Query= "SELECT ID_Richiesta, Prezzo, Budget, Tipo, ID_Cliente FROM RICHIESTE WHERE ID_Richiesta=$id_r;";
   $Res= mysqli_query($Connessione, $Query);
    $Dati=mysqli_fetch_array($Res);
    if($Dati['ID_Richiesta']!=$id_r){
        print("<h2 style=\"background-color:#fe5d00;\">Richiesta non disponibile<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
        exit;
    }
    if($Dati['Tipo']=='Acquisto'){
        $provvigione=($Dati['Budget']/100*15);
    }else{
        $provvigione=($Dati['Prezzo']/100*15);
    }
  //Impostare la query
  $Query= "UPDATE RICHIESTE SET ID_Societa=1, Percentuale=$percentuale, Scadenza='$scadenza', Provvigione=$provvigione, Data='$data' WHERE ID_Richiesta=$id_r;";

  $Res= mysqli_query($Connessione, $Query); //Eseguire la query
    if(!$Res){
  print("<h2 style=\"background-color:#fe5d00;\">Firma richiesta fallita<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
  exit;}

  $Query= "UPDATE SOCIETA SET Patrimonio=(Patrimonio+$provvigione) WHERE ID_Societa=1;";
  ...
  exit;}
    ...
    $id_c=$Dati['ID_Cliente'];
  $Query= "UPDATE CLIENTI SET Patrimonio=(Patrimonio-$provvigione) WHERE ID_Cliente=$id_c;";    
  $Res= mysqli_query($Connessione, $Query); //Eseguire la query
  if(!$Res)
  print("<h2 style=\"background-color:#fe5d00;\">Firma richiesta fallita<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
  else
  print("<h2 style=\"background-color:#fe5d00;\">Richiesta firmata con successo<br/><br/><br/><br/><br/><br/><br/><br/><br/></h2>");
  mysqli_close($Connessione); //Chiudere la connessione
  ?>
