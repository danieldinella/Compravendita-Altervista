...
<!NAVBAR>
  ...
  exit;
    ...
    if($id!=NULL){
        $Sql= "SELECT * FROM RICHIESTE WHERE ID_Cliente='$id';";
     }else{
  ...
  echo("<h2 style=\"background-color:#fe5d00;\">Storico richieste<p></p>
  ...
  <td> Scadenza </td>
  <td> Tipo </td>
  <td> ID Societ&agrave; </td>
  <td> ID Cliente </td>
  <td> ID Titolo </td></tr></h2>");

  while ($Dati = mysqli_fetch_array($Result))
  {
  echo("<tr><td>". $Dati['ID_Richiesta'] . "</td>");
  echo("<td>". $Dati['Percentuale'] . "%</td>");
  echo("<td>&euro;". $Dati['Provvigione'] . "</td>");
  echo("<td>&euro;". $Dati['Prezzo'] . "</td>");
  echo("<td>&euro;". $Dati['Budget'] . "</td>");
  echo("<td>". $Dati['Fido'] . "</td>");
  echo("<td>". $Dati['Data'] . "</td>");
  echo("<td>". $Dati['Scadenza'] . "</td>");
  echo("<td>". $Dati['Tipo'] . "</td>");
  echo("<td>". $Dati['ID_Societa'] . "</td>");
