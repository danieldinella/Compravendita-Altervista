...
<!NAVBAR>
  ...
  $Sql= "SELECT ID_Richiesta, Budget, Data, Scadenza, Provvigione FROM RICHIESTE WHERE RICHIESTE.ID_Societa=1 AND RICHIESTE.ID_Cliente<>$id AND RICHIESTE.Tipo='Acquisto';"; //Impostare la query
  if(!($Result=mysqli_query($Connessione, $Sql))) //Eseguire la query
  print("Query fallita");
  else
  {
  // Formattare il risultato della query in formato xhtml
  echo("<h2 style=\"background-color:#fe5d00;\">Annunci di acquisto<p></p>
  <table width=\"100%\" height=\"auto\" style= \"background-color:#fe5d00\">
  <tr style = \"font-weight: bold\">
  <td> ID Richiesta </td>
  <td> Budget </td>
  <td> Data </td>
  <td> Scadenza </td></tr></h2>");

  while ($Dati = mysqli_fetch_array($Result))
  {
  echo("<tr><td>". $Dati['ID_Richiesta'] . "</td>");
  echo("<td>&euro;". ($Dati['Budget']-$Dati['Provvigione']) . "</td>");
