...
<!NAVBAR>
  ...
  $Sql= "SELECT ID_Richiesta, Prezzo, Data, RICHIESTE.ID_Titolo, Scadenza, Provincia, TITOLI.Tipo FROM RICHIESTE,TITOLI WHERE RICHIESTE.ID_Societa=1 AND RICHIESTE.ID_Cliente<>$id AND RICHIESTE.Tipo='Vendita' AND TITOLI.ID_Titolo=RICHIESTE.ID_Titolo;"; //Impostare la query
  if(!($Result=mysqli_query($Connessione, $Sql))) //Eseguire la query
  print("Query fallita");
  else
  {
  // Formattare il risultato della query in formato xhtml
  echo("<h2 style=\"background-color:#fe5d00;\">Annunci di vendita<p></p>
  <table width=\"100%\" height=\"auto\" style= \"background-color:#fe5d00\">
  ...
  {
  echo("<tr><td>". $Dati['ID_Richiesta'] . "</td>");
  echo("<td>&euro;". $Dati['Prezzo'] . "</td>");
  echo("<td>". $Dati['Data'] . "</td>");
  echo("<td>". $Dati['Scadenza'] . "</td>");
  echo("<td>". $Dati['ID_Titolo'] . "</td>");
