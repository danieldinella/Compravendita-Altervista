...
<!NAVBAR>
  ...
  $Sql= "SELECT ID_Titolo, Tipo, Provincia FROM TITOLI WHERE ID_Cliente='$id';"; //Impostare la query
  if(!($Result=mysqli_query($Connessione, $Sql))) //Eseguire la query
  print("Query fallita");
  else
  {
      if($id==NULL){
  // Formattare il risultato della query in formato xhtml
  echo("<h2 style=\"background-color:#fe5d00;\">Portafoglio titoli societ&agrave;
  <p style=\"font-size:20px;\">Totale = ".mysqli_num_rows($Result)."</p>
  <table width=\"50%\" height=\"auto\" style= \"background-color:#fe5d00\">
  <tr style = \"font-weight: bold\">
  <td> ID Titolo </td>
  <td> Tipo </td>
  <td> Provincia </td></tr></h2>");}else{
      // Formattare il risultato della query in formato xhtml
  echo("<h2 style=\"background-color:#fe5d00;\">Portafoglio titoli cliente
  <p style=\"font-size:20px;\">Totale = ".mysqli_num_rows($Result)."</p>
  <table width=\"50%\" height=\"auto\" style= \"background-color:#fe5d00\">
  <tr style = \"font-weight: bold\">
  <td> ID Titolo </td>
  <td> Tipo </td>
  <td> Provincia </td></tr></h2>");
  }

  while ($Dati = mysqli_fetch_array($Result))
  {
  echo("<tr><td>". $Dati['ID_Titolo'] . "</td>");
  echo("<td>". $Dati['Tipo'] . "</td>");
  echo("<td>". $Dati['Provincia'] . "</td>");
  }
  echo("</table><br/><br/><br/><br/><br/>");
  }
    if($id==NULL){
  $Sql= "SELECT ID_Titolo, Tipo, Provincia, ID_Cliente FROM TITOLI WHERE ID_Cliente!=0;"; //Impostare la query
  if(!($Result=mysqli_query($Connessione, $Sql))) //Eseguire la query
  print("Query fallita");
  else
  {
  // Formattare il risultato della query in formato xhtml
  echo("<h2 style=\"background-color:#fe5d00;\">Portafoglio titoli clienti
  <p style=\"font-size:20px;\">Totale = ".mysqli_num_rows($Result)."</p>
  <table width=\"50%\" height=\"auto\" style= \"background-color:#fe5d00\">
  <tr style = \"font-weight: bold\">
  <td> ID Titolo </td>
